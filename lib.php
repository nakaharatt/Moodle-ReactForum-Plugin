...
function reactforum_add_instance($reactforum, $mform = null) {
    ...
    \core_completion\api::update_completion_date_event($reactforum->coursemodule, 'reactforum', $reactforum->id, $completiontimeexpected);
...
function reactforum_cron() {
    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            if (get_user_preferences('reactforum_markasreadonnotification', 1, $userto->id) == 1) {
...
function reactforum_filter_user_groups_discussions($discussions) {
    ...
    debugging('The function reactforum_filter_user_groups_discussions() is now deprecated.', DEBUG_DEVELOPER);
...
function reactforum_print_overview($courses,&$htmlarray) {
    ...
    debugging('The function reactforum_print_overview() is now deprecated.', DEBUG_DEVELOPER);
...
function reactforum_search_posts($searchterms, $courseid=0, $limitfrom=0, $limitnum=50,
                ...
                for ($i = 0; $i <= substr_count($token->getValue(), ','); $i++) {
                    ...
                    $tagjoins .= " LEFT JOIN {tag_instance} ti_$tagfieldcount
                                        ON p.id = ti_$tagfieldcount.itemid
                                            AND ti_$tagfieldcount.component = 'mod_reactforum'
                                            AND ti_$tagfieldcount.itemtype = 'reactforum_posts'";
    ...
    $fromsql = "{reactforum_posts} p
                  INNER JOIN {reactforum_discussions} d ON d.id = p.discussion
...
function reactforum_get_discussions($cm, $reactforumsort="", $fullpost=true, $unused=-1, $limit=-1,
    ...
    if (!has_capability('mod/reactforum:viewdiscussion', $modcontext)) { /// User must have perms to view discussions
        return array();
    }
...
function reactforum_print_post($post, $discussion, $reactforum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if (\core_tag_tag::is_enabled('mod_reactforum', 'reactforum_posts')) {
        $postcontent .= $OUTPUT->tag_list(core_tag_tag::get_item_tags('mod_reactforum', 'reactforum_posts', $post->id), null, 'reactforum-tags');
    }
    ...
    if ($istracked && !$CFG->reactforum_usermarksread && !$postisread) {
        reactforum_tp_mark_post_read($USER->id, $post);
    }
...
function reactforum_search_form($course, $search='') {
    ...
    return $output->render($reactforumsearch);
}
...
function reactforum_add_new_post($post, $mform, $unused = null) {
    ...
    if (reactforum_tp_can_track_reactforums($reactforum) && reactforum_tp_is_tracked($reactforum)) {
        reactforum_tp_mark_post_read($post->userid, $post);
    }

    if (isset($post->tags)) {
        core_tag_tag::set_item_tags('mod_reactforum', 'reactforum_posts', $post->id, $context, $post->tags);
    }
...
function reactforum_update_post($newpost, $mform, $unused = null) {
    ...
    if (isset($newpost->tags)) {
        core_tag_tag::set_item_tags('mod_reactforum', 'reactforum_posts', $post->id, $context, $newpost->tags);
    }
...
function reactforum_add_discussion($discussion, $mform=null, $unused=null, $userid=null) {
    ...
    if (isset($discussion->tags)) {
        core_tag_tag::set_item_tags('mod_reactforum', 'reactforum_posts', $post->id, context_module::instance($cm->id), $discussion->tags);
    }
 ...
 * @return string
 */
function reactforum_get_subscribe_link($reactforum, $context, $messages = array(), $cantaccessagroup = false, $unused1 = true,
...
function reactforum_user_can_post($reactforum, $discussion, $user=NULL, $cm=NULL, $course=NULL, $context=NULL) {
    ...
    if (reactforum_discussion_is_locked($reactforum, $discussion)) {
        if (!has_capability('mod/reactforum:canoverridediscussionlock', $context)) {
            return false;
        }
    }
...
function reactforum_user_can_see_post($reactforum, $discussion, $post, $user=NULL, $cm=NULL) {
    ...
    $canviewdiscussion = (isset($cm->cache) && !empty($cm->cache->caps['mod/reactforum:viewdiscussion']))
        || has_capability('mod/reactforum:viewdiscussion', $modcontext, $user->id);
    ...
    if ($reactforum->type == 'qanda') {
        if (has_capability('mod/reactforum:viewqandawithoutposting', $modcontext, $user->id) || $post->userid == $user->id
...
function reactforum_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    ...
    $discussionurl = new moodle_url('/mod/reactforum/discuss.php', ['d' => $content->discussion]);
    ...
    if ($authorhidden) {
        ...
        $by->date = $timestamp;
        $authornamedate = get_string('bynameondate', 'reactforum', $by);
    }
...
function reactforum_update_subscriptions_button($courseid, $reactforumid) {
    ...
    if (!empty($USER->subscriptionsediting)) {
        $string = get_string('managesubscriptionsoff', 'reactforum');
        $edit = "off";
    } else {
        $string = get_string('managesubscriptionson', 'reactforum');
        $edit = "on";
    }

    $subscribers = html_writer::start_tag('form', array('action' => $CFG->wwwroot . '/mod/reactforum/subscribers.php',
        'method' => 'get', 'class' => 'form-inline'));
    $subscribers .= html_writer::empty_tag('input', array('type' => 'submit', 'value' => $string,
        'class' => 'btn btn-secondary'));
    $subscribers .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'id', 'value' => $reactforumid));
 ...
 * @return bool
 */
function reactforum_tp_mark_post_read($userid, $post, $unused = null) {
 ...
 * @return int
 */
function reactforum_tp_count_reactforum_unread_posts($cm, $course, $resetreadcache = false) {
    global $CFG, $USER, $DB;
    ...
    if ($canmanage) {
        ...
        switch ($subscriptionmode) {
            case REACTFORUM_CHOOSESUBSCRIBE : // 0
                ...
                $allowchoice->icon = new pix_icon('t/selected', '', 'mod_reactforum');
                break;
            case REACTFORUM_FORCESUBSCRIBE : // 1
                $forceforever->action = null;
                $forceforever->add_class('activesetting');
                $forceforever->icon = new pix_icon('t/selected', '', 'mod_reactforum');
                break;
            case REACTFORUM_INITIALSUBSCRIBE : // 2
                $forceinitially->action = null;
                $forceinitially->add_class('activesetting');
                $forceinitially->icon = new pix_icon('t/selected', '', 'mod_reactforum');
                break;
            case REACTFORUM_DISALLOWSUBSCRIBE : // 3
                $disallowchoice->action = null;
                $disallowchoice->add_class('activesetting');
                $disallowchoice->icon = new pix_icon('t/selected', '', 'mod_reactforum');
                break;
        }
 ...
 *
 * @param object $post The reactforum post.
 * @param object $reactforum The reactforum object.
 * @return bool
 * @throws coding_exception
 */
function reactforum_is_author_hidden($post, $reactforum) {
    ...
    if (!isset($reactforum->type)) {
        throw new coding_exception('$reactforum->type must be set.');
    }
    if ($reactforum->type === 'single' && empty($post->parent)) {
        return true;
    }
    return false;
}
...
function mod_reactforum_inplace_editable($itemtype, $itemid, $newvalue) {
    ...
    if ($itemtype === 'digestoptions') {
        ...
        reactforum_set_user_maildigest($reactforum, $newvalue);

        $renderer = $PAGE->get_renderer('mod_reactforum');
        return $renderer->render_digest_options($reactforum, $newvalue);
    }
}
...
function reactforum_discussion_is_locked($reactforum, $discussion) {
    ...
    if ($reactforum->type === 'single') {
        // It does not make sense to lock a single discussion reactforum.
        return false;
    }

    if (($discussion->timemodified + $reactforum->lockdiscussionafter) < time()) {
        return true;
    }
...
/**
 * Check if the user can create attachments in a reactforum.
 * @param  stdClass $reactforum   reactforum object
 * @param  stdClass $context context object
 * @return bool true if the user can create attachments, false otherwise
 * @since  Moodle 3.3
 */
function reactforum_can_create_attachment($reactforum, $context) {
    // If maxbytes == 1 it means no attachments at all.
    if (empty($reactforum->maxattachments) || $reactforum->maxbytes == 1 ||
            !has_capability('mod/reactforum:createattachment', $context)) {
        return false;
    }
    return true;
}
 ...
 * @return  array
 */
function mod_reactforum_get_fontawesome_icon_map() {
    return [
        'mod_reactforum:i/pinned' => 'fa-map-pin',
        'mod_reactforum:t/selected' => 'fa-check',
        'mod_reactforum:t/subscribed' => 'fa-envelope-o',
        'mod_reactforum:t/unsubscribed' => 'fa-envelope-open-o',
    ];
}
 ...
 * @return bool
 */
function mod_reactforum_core_calendar_event_action_shows_item_count(calendar_event $event, $itemcount = 0) {
    // Always show item count for reactforums if item count is greater than 1.
 ...
 */
function mod_reactforum_core_calendar_provide_event_action(calendar_event $event,
                                                       \core_calendar\action_factory $factory) {
    global $DB, $USER;

    $cm = get_fast_modinfo($event->courseid)->instances['reactforum'][$event->instance];
    $context = context_module::instance($cm->id);
    ...
    $itemcount = 0;
    $reactforum = $DB->get_record('reactforum', array('id' => $cm->instance));
    $postcountsql = "
                SELECT
                    COUNT(1)
                  FROM
                 ...
                 WHERE
                    fp.userid=:userid AND fd.reactforum=:reactforumid";
    $postcountparams = array('userid' => $USER->id, 'reactforumid' => $reactforum->id);

    if ($reactforum->completiondiscussions) {
        $count = $DB->count_records('reactforum_discussions', array('reactforum' => $reactforum->id, 'userid' => $USER->id));
        $itemcount += ($reactforum->completiondiscussions >= $count) ? ($reactforum->completiondiscussions - $count) : 0;
    }

    if ($reactforum->completionreplies) {
        $count = $DB->get_field_sql( $postcountsql.' AND fp.parent<>0', $postcountparams);
        $itemcount += ($reactforum->completionreplies >= $count) ? ($reactforum->completionreplies - $count) : 0;
    }

    if ($reactforum->completionposts) {
        $count = $DB->get_field_sql($postcountsql, $postcountparams);
        $itemcount += ($reactforum->completionposts >= $count) ? ($reactforum->completionposts - $count) : 0;
    }

    // Well there is always atleast one actionable item (view reactforum, etc).
    $itemcount = $itemcount > 0 ? $itemcount : 1;

    return $factory->create_instance(
        get_string('view'),
        new \moodle_url('/mod/reactforum/view.php', ['id' => $cm->id]),
        $itemcount,
        true
    );
}
...
/**
 * Add a get_coursemodule_info function in case any reactforum type wants to add 'extra' information
 ...
 */
function reactforum_get_coursemodule_info($coursemodule) {
    global $DB;

    $dbparams = ['id' => $coursemodule->instance];
    $fields = 'id, name, intro, introformat, completionposts, completiondiscussions, completionreplies';
    if (!$reactforum = $DB->get_record('reactforum', $dbparams, $fields)) {
        return false;
    }

    $result = new cached_cm_info();
    $result->name = $reactforum->name;

    if ($coursemodule->showdescription) {
        // Convert intro to html. Do not filter cached version, filters run at display time.
        $result->content = format_module_intro('reactforum', $reactforum, $coursemodule->id, false);
    }

    // Populate the custom completion rules as key => value pairs, but only if the completion mode is 'automatic'.
    if ($coursemodule->completion == COMPLETION_TRACKING_AUTOMATIC) {
        $result->customdata['customcompletionrules']['completiondiscussions'] = $reactforum->completiondiscussions;
        $result->customdata['customcompletionrules']['completionreplies'] = $reactforum->completionreplies;
        $result->customdata['customcompletionrules']['completionposts'] = $reactforum->completionposts;
    }
 ...
 */
function mod_reactforum_get_completion_active_rule_descriptions($cm) {
